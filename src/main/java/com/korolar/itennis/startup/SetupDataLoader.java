...
public class SetupDataLoader implements ApplicationListener<ContextRefreshedEvent> {
	...
	public void onApplicationEvent(ContextRefreshedEvent event) {
		...
		schedule.setBeginning(LocalDateTime.now().minusHours(10));
		schedule.setEnd(LocalDateTime.now().minusHours(8));
		schedule.setValue(BigDecimal.TEN);
		schedule.setClub(club1);
		schedule.setPlayed(Boolean.FALSE);

		Schedule schedule2 = new Schedule();
		schedule2.setBeginning(LocalDateTime.now().minusHours(9));
		schedule2.setEnd(LocalDateTime.now().minusHours(7));
		schedule2.setValue(BigDecimal.TEN);
		schedule2.setClub(club1);
		schedule2.setPlayed(Boolean.FALSE);


		Schedule schedule1 = new Schedule();
		schedule1.setBeginning(LocalDateTime.now().minusHours(6));
		schedule1.setEnd(LocalDateTime.now().minusHours(5));
		schedule1.setValue(BigDecimal.TEN);
		schedule1.setClub(club1);
		schedule1.setPlayed(Boolean.TRUE);

		Schedule schedule3 = new Schedule();
		schedule3.setBeginning(LocalDateTime.now().minusHours(3));
		schedule3.setEnd(LocalDateTime.now().minusHours(2));
		schedule3.setValue(BigDecimal.TEN);
		schedule3.setClub(club1);
